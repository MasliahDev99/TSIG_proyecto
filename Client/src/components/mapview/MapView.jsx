...
const userLocationStyle = new Style({
  image: new CircleStyle({
    radius: 7,
    fill: new Fill({ color: "rgba(0, 0, 255, 0.6)" }), // Azul semitransparente
...
// Agregar estos estilos después de los existentes
...
const polygonCompleteStyle = new Style({
  stroke: new Stroke({
...
export default function MapView({
  ...
  // Agregar estos refs después de los existentes
  const polygonDrawLayerRef = useRef(null)
  const polygonDrawInteractionRef = useRef(null)
  ...
  const visibleLineIdsRef = useRef(new Set())
  ...
  useEffect(() => {
    ...
    stopLayerRef.current = new VectorLayer({
      source: stopSource,
      style: (feature) => {
        const habilitada = feature.get("habilitada") ?? feature.get("activa") ?? true
        const estado = feature.get("estado") ?? false
        const noModificable = feature.get("no_modificable") === true

        // Color basado en 'estado': verde si tiene líneas asociadas, rojo si no
        const fillColor = estado ? "rgba(34, 197, 94, 0.7)" : "rgba(239, 68, 68, 0.7)"

        // Si no está habilitada, usar opacidad reducida
        const opacity = habilitada ? 0.7 : 0.4

        // Borde especial para paradas no modificables
        const strokeColor = noModificable ? "rgba(255, 165, 0, 1)" : "white"
        const strokeWidth = noModificable ? 2.5 : 1.5

        return new Style({
          image: new CircleStyle({
            radius: 6,
            fill: new Fill({
              color: fillColor.replace("0.7", opacity.toString()),
            }),
    ...
    // Agregar después de la inicialización del mapa (dentro del useEffect principal)
    // Capa para dibujo de polígono
    polygonDrawLayerRef.current = new VectorLayer({
      source: new VectorSource(),
      style: polygonDrawStyle,
      zIndex: 2, // Muy alta para estar encima de todo
    })
    ...
    map.addLayer(userLocationLayerRef.current)
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const transformedCoords = transform([lon, lat], "EPSG:4326", "EPSG:32721")
        console.log("🎯 Mapa centrado en ubicación del usuario:", transformedCoords)

        const view = map.getView()
        view.animate({ center: transformedCoords, zoom: 14 })

        const feature = new Feature({
          geometry: new Point(transformedCoords),
        })

        userLocationFeatureRef.current = feature
        userLocationLayerRef.current.getSource().clear()
        userLocationLayerRef.current.getSource().addFeature(feature)
      },
      (error) => {
  ...
  const checkAndSendAttributes = () => {
    ...
    if (
      ...
      // 🔵 Rutas
      const rutasFeatures = camineriaSource.getFeatures().filter((f) => {
  ...
  const detectLocationInfo = (coordinate) => {
    ...
    return { department: department || "", city: city || "Sin ciudad" }
  }
  ...
  useEffect(() => {
    ...
    const clickHandler = (evt) => {
      // PRIMERA VERIFICACIÓN: Si estamos dibujando polígono, no procesar clics normales
      if (isDrawingPolygonRef.current) {
        console.log("🖊️ Dibujando polígono, ignorando clic normal")
        return
      }
      ...
      if (isAdmin) {
        ...
        if (activeTool === "add-stop") {
          ...
          // Solo permitir clics en paradas para routing punto a punto
          if (featureClicked && layerClicked === stopLayerRef.current) {
            // Verificar que la parada esté habilitada usando el campo correcto
            const stopData = featureClicked.getProperties()
            const isEnabled = stopData.habilitada ?? true // Usar 'habilitada' en lugar de 'estado' o 'activa'

            if (!isEnabled) {
              alert("⚠️ No se puede seleccionar una parada deshabilitada para crear líneas.")
              return
            }

            const stopCoordinate = featureClicked.getGeometry().getCoordinates()
            const locationInfo = detectLocationInfo(stopCoordinate)

            console.log("🚏 Parada seleccionada para routing:", stopData.nombre || stopData.name)
            console.log("🏛️ Información de ubicación detectada:", locationInfo)
            ...
            onMapInteractionRequest({
              ...
              payload: {
                ...
                detectedDepartment: locationInfo.department,
                detectedCity: locationInfo.city,
                stopData: stopData, // Incluir datos completos de la parada
              },
            })
          } else {
            // Si no se hizo clic en una parada, mostrar mensaje
            alert("⚠️ Para crear una línea punto a punto, debe seleccionar paradas existentes en el mapa.")
            return
          }
          ...
          const stopData = featureClicked.getProperties()
          const noModificable = stopData.no_modificable === true

          if (noModificable) {
            alert("❌ Esta parada no se puede eliminar porque es destino o partida de por lo menos una linea.")
            return
          }
        ...
        if (featureClicked) {
          ...
          // Mejorar el manejo de nombres para evitar "undefined"
          let featureName = "Sin nombre"
          if (props.nombre) {
            featureName = props.nombre
          } else if (props.name) {
            featureName = props.name
          } else if (props.descripcion) {
            featureName = props.descripcion
          } else if (props.id) {
            featureName = `ID: ${props.id}`
          } else if (props.id_linea) {
            featureName = `Línea ID: ${props.id_linea}`
          }

          let content = `${featureType}: ${featureName}\n`
          ...
          if (featureType === "Parada") {
            ...
            content += `Coords UTM (X/Y): ${utmCoords[0]}, ${utmCoords[1]}`
          } else if (featureType === "Línea") {
            content += `ID Línea: ${props.id_linea || "N/D"}\n`
            content += `Descripción: ${props.descripcion || "N/D"}\n`
            content += `Coords UTM (X/Y): ${utmCoords[0]}, ${utmCoords[1]}`
          } else {
            for (const key in props) {
              if (key !== "geometry" && Object.prototype.hasOwnProperty.call(props, key)) {
  ...
  useEffect(() => {
    const handleStartEdit = (e) => {
      ...
      if (stopLayerRef.current) {
        const features = stopLayerRef.current.getSource().getFeatures()
        const stopFeature = features.find((f) => {
          const stopId = f.get("id_parada") || f.get("id")
          return stopId == id
        })
      }
  ...
  useEffect(() => {
    const handleStartLinkStops = (e) => {
      ...
      if (routeCoordinates && Array.isArray(routeCoordinates) && routeCoordinates.length > 0) {
        ...
        if (routeCoordinates[0] && Array.isArray(routeCoordinates[0])) {
          console.log("  - Contenido del primer elemento:", routeCoordinates[0])
        }
      }
  ...
  // Manejo del dibujo de polígono
  useEffect(() => {
    const handleStartPolygonDraw = () => {
      if (!mapInstanceRef.current || !polygonDrawLayerRef.current) return

      console.log("🖊️ Iniciando dibujo de polígono")
      isDrawingPolygonRef.current = true

      // Limpiar polígono anterior
      polygonDrawLayerRef.current.getSource().clear()

      // Crear interacción de dibujo
      const drawInteraction = new Draw({
        source: polygonDrawLayerRef.current.getSource(),
        type: "Polygon",
        style: polygonDrawStyle,
      })

      polygonDrawInteractionRef.current = drawInteraction
      mapInstanceRef.current.addInteraction(drawInteraction)

      // Manejar finalización del dibujo
      drawInteraction.on("drawend", (event) => {
        console.log("✅ Polígono completado")

        const feature = event.feature
        const geometry = feature.getGeometry()
        const coordinates = geometry.getCoordinates()[0] // Primer anillo del polígono

        // Transformar coordenadas de UTM a WGS84 para Turf.js
        const wgs84Coordinates = coordinates.map((coord) => transform(coord, "EPSG:32721", "EPSG:4326"))

        // Cambiar estilo a completado
        feature.setStyle(polygonCompleteStyle)

        // Remover interacción de dibujo
        mapInstanceRef.current.removeInteraction(drawInteraction)
        polygonDrawInteractionRef.current = null
        isDrawingPolygonRef.current = false

        // Enviar evento con las coordenadas
        window.dispatchEvent(
          new CustomEvent("polygon-drawn", {
            detail: { coordinates: wgs84Coordinates },
          }),
        )

        console.log("📐 Coordenadas del polígono:", wgs84Coordinates)
      })

      // Manejar cancelación
      drawInteraction.on("drawabort", () => {
        console.log("❌ Dibujo de polígono cancelado")
        isDrawingPolygonRef.current = false
      })
    }

    const handleStopPolygonDraw = () => {
      if (!mapInstanceRef.current) return

      console.log("🛑 Deteniendo dibujo de polígono")
      isDrawingPolygonRef.current = false

      // Remover interacción si existe
      if (polygonDrawInteractionRef.current) {
        mapInstanceRef.current.removeInteraction(polygonDrawInteractionRef.current)
        polygonDrawInteractionRef.current = null
      }
    }

    const handleClearPolygonFilter = () => {
      if (!polygonDrawLayerRef.current) return

      console.log("🧹 Limpiando filtro de polígono")

      // Limpiar la capa
      polygonDrawLayerRef.current.getSource().clear()

      // Detener dibujo si está activo
      handleStopPolygonDraw()

      // Enviar evento de limpieza
      window.dispatchEvent(new CustomEvent("polygon-cleared"))
    }

    window.addEventListener("start-polygon-draw", handleStartPolygonDraw)
    window.addEventListener("stop-polygon-draw", handleStopPolygonDraw)
    window.addEventListener("clear-polygon-filter", handleClearPolygonFilter)

    return () => {
      window.removeEventListener("start-polygon-draw", handleStartPolygonDraw)
      window.removeEventListener("stop-polygon-draw", handleStopPolygonDraw)
      window.removeEventListener("clear-polygon-filter", handleClearPolygonFilter)

      // Limpiar al desmontar
      if (mapInstanceRef.current && polygonDrawInteractionRef.current) {
        mapInstanceRef.current.removeInteraction(polygonDrawInteractionRef.current)
      }
    }
  }, [])
  ...
  useEffect(() => {
    const handleCenterOnLocation = (event) => {
      const { coordinates } = event.detail

      if (mapInstanceRef.current && coordinates) {
        const view = mapInstanceRef.current.getView()

        // Centrar el mapa en las coordenadas UTM
        view.animate({
          center: coordinates,
          zoom: 15, // Zoom cercano para mostrar el área local
          duration: 1000, // Animación de 1 segundo
        })

        console.log("🎯 Mapa centrado en ubicación del usuario:", coordinates)
      }
    }

    window.addEventListener("center-map-on-location", handleCenterOnLocation)

    return () => {
      window.removeEventListener("center-map-on-location", handleCenterOnLocation)
    }
  }, [])
  ...
  const handleStopDelete = async (stopId) => {
    try {
      // Verificar si la parada es modificable antes de eliminar
      if (stopLayerRef.current) {
        const features = stopLayerRef.current.getSource().getFeatures()
        const stopFeature = features.find((f) => {
          const fStopId = f.get("id_parada") || f.get("id")
          return fStopId == stopId
        })

        if (stopFeature) {
          const noModificable = stopFeature.get("no_modificable") === true
          if (noModificable) {
      ...
      if (response.status === 200) {
        ...
        window.dispatchEvent(new CustomEvent("stop-deleted", { detail: { stopId } }))
        window.dispatchEvent(new CustomEvent("end-edit-mode"))
        window.dispatchEvent(new CustomEvent("refresh-map-data"))
      } else {
        console.error("❌ Error al eliminar parada:", response.status, response.data)
        alert(`Error al eliminar parada: ${response.statusText}`)
      }
    } catch (error) {
