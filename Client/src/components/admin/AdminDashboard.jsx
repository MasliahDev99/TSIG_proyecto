  ...
  const handleMapInteraction = (interaction) => {
    ...
    if (selectedTool === "add-stop" && interaction.type === "MAP_CLICK_FOR_ADD") {
      ...
      setPopupFormConfig({
        ...
        coordinates: interaction.payload.coordinates,
        initialData: {
          ...interaction.payload.featureData,
          CiudadOrigen: interaction.payload.featureData.ciudad_origen,
          CiudadDestino: interaction.payload.featureData.ciudad_destino,
          DeptoOrigen: interaction.payload.featureData.depto_origen,
          DeptoDestino: interaction.payload.featureData.depto_destino,
        },
      ...
      const newPoint = {
        ...
        coordinates: interaction.payload.coordinates,
        type: "routing",
        stopData: interaction.payload.stopData, // Incluir datos completos de la parada
      }

      const updatedPoints = [...currentLinePoints, newPoint]
      setCurrentLinePoints(updatedPoints)

      // Agregar la parada a la lista de paradas seleccionadas
      const newStop = {
        id: interaction.payload.stopData.id_parada || interaction.payload.stopData.id,
        name: interaction.payload.stopData.nombre || interaction.payload.stopData.name,
        coordinates: interaction.payload.coordinates,
        type: "stop",
        department: interaction.payload.detectedDepartment,
        city: interaction.payload.detectedCity,
      }

      const updatedStops = [...currentLineStops, newStop]
      setCurrentLineStops(updatedStops)

      // Detectar departamento para cada punto
      const department = interaction.payload.detectedDepartment || ""
      const city = interaction.payload.detectedCity || ""

      console.log(
        "🏛️ Departamento detectado para parada de routing:",
        department,
        "en coordenadas:",
      ...
      if (updatedPoints.length === 2) {
        ...
        setPopupFormConfig((prev) => ({
          ...prev,
          message: `Parada ${updatedPoints.length}/2 seleccionada: ${interaction.payload.stopData?.nombre || interaction.payload.stopData?.name || "Sin nombre"}. ${updatedPoints.length === 1 ? "Seleccione la parada de destino." : ""}`,
        }))
      }
    } else if (
  ...
  useEffect(() => {
    const handleRouteCalculated = (event) => {
      ...
      if (success && route) {
        ...
        // Mantener las paradas seleccionadas
        const updatedStops = currentLineStops // Declare updatedStops here
        setCurrentLineStops(updatedStops)
        ...
        setTimeout(() => {
          setPopupFormConfig({
            ...
            initialData: {
              stops: updatedStops, // Usar las paradas que se seleccionaron
            ...
            message:
              "✅ Ruta calculada. Complete los datos de la línea y haga clic en paradas cercanas (hasta 25m) para vincular más paradas.",
            key: Date.now(),
          })
          ...
          console.log("🚏 Paradas iniciales:", updatedStops.length)
        }, 100)
      } else {
        setPopupFormConfig((prev) => ({
          ...prev,
