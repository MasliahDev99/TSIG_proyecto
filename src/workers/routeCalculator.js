...
export const useRouteCalculator = () => {
  ...
  // Hook simplificado ya que ahora el cálculo se hace directamente en el componente
  const calculateRouteDirect = useCallback(() => Promise.resolve({ success: false, message: "Not implemented" }), [])
  const cancelCalculationDirect = useCallback(() => {}, [])
  const cleanupDirect = useCallback(() => {}, [])

  return {
    calculateRoute: calculateRouteDirect,
    cancelCalculation: cancelCalculationDirect,
    cleanup: cleanupDirect,
    isCalculating,
    progress,
    progressMessage,
  }
